...
def create_order(request):
    data = request.data
    data["placed_by"] = request.auth["merchant_id"]
    ...
    if "add_products" in data:
        ...
        for product in products:
            ...
            if product_instance not in product_set:
                ...
                total = float(
                    product["quantity"] * product_instance.inventory.store_price
                )
                ...
                order_detail_instance = OrderDetail.objects.get(
                    order=order_instance, item_code=product_instance
                )
...
def delete_order(request):
    ...
    try:
        ...
        return Response(
            {"message": "Order Already Deleted"}, status=status.HTTP_204_NO_CONTENT
        )
